# hvit/models/dfca.py
"""
Disease-Focused Cross-Attention (DFCA) module.

This module provides cross-attention based fusion of RGB and spectral features
with optional learnable disease-specific attention biasing.
"""
...
class DiseaseFocusedCrossAttention(nn.Module):
    ...
    Args:
        ...
        use_disease_mask: Whether to use learnable disease mask parameter.
    ...
    def __init__(
        ...
        use_disease_mask: bool = True
    ) -> None:
        super().__init__()
        if embed_dim % num_heads != 0:
            raise ValueError(
                f"DFCA Error: embed_dim ({embed_dim}) must be divisible by num_heads ({num_heads})."
            )
        ...
        if self.use_disease_mask:
            self.disease_mask_param = nn.Parameter(torch.zeros(1, 1, embed_dim))
        ...
        logger.info(
            f"DiseaseFocusedCrossAttention module initialized: embed_dim={embed_dim}, "
            f"num_heads={num_heads}, dropout={dropout_rate}, use_mask={use_disease_mask}"
        )
    ...
    def forward(
        ...
        Args:
            rgb_features: RGB features [seq_len, batch_size, embed_dim].
            spectral_features: Spectral features [seq_len, batch_size, embed_dim].

        Returns:
            Fused features [seq_len, batch_size, embed_dim].
        """
        spectral_qkv = spectral_features
        if self.use_disease_mask:
            spectral_qkv = spectral_features + self.disease_mask_param

        # Cross-attention: rgb_features are queries, spectral_features are keys and values
