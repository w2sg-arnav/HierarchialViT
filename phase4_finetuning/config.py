...
if not os.path.exists(resolved_ssl_checkpoint_path):
    _config_module_logger.error(f"CRITICAL: SSL Checkpoint path not found: {resolved_ssl_checkpoint_path}. Please verify.")
...
HVT_ARCH_PARAMS_FOR_FINETUNE = phase3_cfg.get('hvt_params_for_backbone', _default_hvt_arch).copy()
if HVT_ARCH_PARAMS_FOR_FINETUNE == _default_hvt_arch:
     _config_module_logger.warning("Using FALLBACK HVT architecture for fine-tuning. VERIFY!")
...
config = {
    ...
    "log_dir": "logs_finetune_optimized_v1", 
    "log_file_finetune": "finetune_hvt_xl_optimized_v1.log",
    "best_model_filename": "best_finetuned_hvt_xl_optimized_v1.pth",
    "final_model_filename": "final_finetuned_hvt_xl_optimized_v1.pth",
    "checkpoint_save_dir_name": "checkpoints_optimized_v1",
    ...
    "train_split_ratio": DEFAULT_TRAIN_SPLIT_RATIO_FINETUNE, # Now defined
    "normalize_data": True,
    ...
    "model_architecture_name": "DiseaseAwareHVT_SSL_OptimizedFinetune",
    ...
    "lr_head_frozen_phase": 1e-3,      
    "lr_backbone_unfrozen_phase": 5e-5, 
...
_config_module_logger.info(f"Phase 4 Config (Optimized Strategy) Loaded. Target: 85%+ Accuracy.")
_config_module_logger.info(f"Pretrained Checkpoint: {config.get('pretrained_checkpoint_path')}")
_config_module_logger.info(f"Total Epochs: {config['epochs']}, Batch Size: {config['batch_size']}, Accumulation: {config['accumulation_steps']}")
_config_module_logger.info(f"Freeze Epochs: {config['freeze_backbone_epochs']}. LR Head (Frozen): {config['lr_head_frozen_phase']:.1e}")
_config_module_logger.info(f"Unfrozen LRs: Backbone={config['lr_backbone_unfrozen_phase']:.1e}, Head={config['lr_head_unfrozen_phase']:.1e}")
_config_module_logger.info(f"HVT Drop Path Rate: {config['hvt_params_for_model_init']['drop_path_rate']}")
