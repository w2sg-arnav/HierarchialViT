...
config = {
    ...
    "log_dir": "logs_finetune_optimized_strategy_v1_stable", # Added _stable
    "log_file_finetune": "finetune_hvt_optimized_strategy_v1_stable.log",
    "best_model_filename": "best_finetuned_hvt_optimized_strategy_v1_stable.pth",
    "final_model_filename": "final_finetuned_hvt_optimized_strategy_v1_stable.pth",
    "checkpoint_save_dir_name": "checkpoints_optimized_strategy_v1_stable",
    ...
    "model_architecture_name": "DiseaseAwareHVT_Optimized_Strategy_Finetune_v1_Stable",
    ...
    "hvt_params_for_model_init": {
        **HVT_ARCH_PARAMS_FROM_SSL_SOURCE,
        "model_drop_rate": 0.1,
        "drop_path_rate": 0.05,
        "use_gradient_checkpointing": True, # Keep True for memory if needed, but can set to False for debugging NaNs
    ...
    "epochs": 150, # Can reduce for faster debug cycles if NaNs are frequent

    # --- STABILITY_UPDATES incorporated ---
    "batch_size": 8,  # Reduced from 12
    "accumulation_steps": 6,  # Increased from 4 (Effective BS 48 maintained)

    "amp_enabled": False,  # CRITICAL: Disable AMP temporarily for NaN debugging
    "clip_grad_norm": 0.5,  # Reduced from 1.0

    "optimizer": "AdamW",
    "optimizer_params": {
        "betas": (0.9, 0.999),
        "eps": 1e-7  # Increased from 1e-8
    },
    "weight_decay": 0.05,

    "lr_head_frozen_phase": 1e-4,       # Reduced from 3e-4
    "lr_backbone_unfrozen_phase": 1e-5, # Reduced from 3e-5
    "lr_head_unfrozen_phase": 1e-4,     # Reduced from 3e-4

    "scheduler": "WarmupCosine",
    "warmup_epochs": 15,  # Increased from 10
    "eta_min_lr": 1e-7, # Can be 1e-6 if needed later

    "loss_label_smoothing": 0.1, # Keep, generally stable
    "augmentations_enabled": True,
    "augmentation_strategy": "stable_enhanced",  # 'stable_enhanced', 'minimal', 'enhanced' (legacy)
    "augmentation_severity": "mild",      # For 'stable_enhanced': 'mild', 'moderate', 'strong'

    "evaluate_every_n_epochs": 1,
    "early_stopping_patience": 25, # Can be reduced for faster debug cycles
    "metric_to_monitor_early_stopping": "f1_macro",

    "tta_enabled_val": True, # Can disable for faster validation during NaN debug
    "ssl_pretrain_img_size_fallback": tuple(phase3_cfg.get('pretrain_img_size', (448,448))),

    # --- DEBUGGING_CONFIG incorporated ---
    "debug_nan_detection": True,
    "stop_on_nan_threshold": 5,  # Stop if more than 5 NaN losses in an epoch (was 10)
    "monitor_gradients": True,
    "gradient_log_interval": 50, # Log gradient norm every 50 optimizer steps
    # "log_model_stats_every_n_batches": 100, # Covered by tqdm and epoch logs
    "save_checkpoint_every_n_epochs": 10, # Periodic checkpoint saving
}

NUM_CLASSES = config['num_classes']

_config_module_logger.info(f"Phase 4 Config (Optimized_Strategy_v1, Stable Variant) Loaded.")
...
_config_module_logger.info(f"Optimizer Eps: {config['optimizer_params']['eps']:.1e}, Clip Grad Norm: {config['clip_grad_norm']}")
...
_config_module_logger.info(f"NaN Detection: {config['debug_nan_detection']}, Stop Threshold: {config['stop_on_nan_threshold']}")
_config_module_logger.info(f"Gradient Monitoring: {config['monitor_gradients']}, Interval: {config['gradient_log_interval']}")