# hvit/models/baseline.py
"""
Baseline models for comparison with HViT.

This module provides InceptionV3 baseline implementation for benchmarking.
"""
...
class InceptionV3Baseline(nn.Module):
    """InceptionV3 baseline model with optional spectral channel support.
    ...
    def __init__(
        ...
        self.img_input_size = model_params.get('img_size', (299, 299))
        ...
        self.inception_model = tv_models.inception_v3(
            weights=weights,
            aux_logits=True,
        ...
        if pretrained:
            with torch.no_grad():
                ...
                if self.spectral_conv1:
                    ...
                    self.spectral_conv1.weight.data.copy_(
                        mean_rgb_weights.repeat(1, self.spectral_channels, 1, 1)
                    )
                    if original_conv1.bias is not None:
        ...
        self.inception_model.fc = nn.Linear(
            self.inception_model.fc.in_features,
            self.num_classes
        )
        ...
        if hasattr(self.inception_model, 'AuxLogits') and self.inception_model.AuxLogits is not None:
            self.inception_model.AuxLogits.fc = nn.Linear(
                self.inception_model.AuxLogits.fc.in_features,
                self.num_classes
            )
        
        logger.info(
            f"InceptionV3Baseline: Adapted for {self.num_classes} classes, "
            f"spectral_channels={self.spectral_channels}."
        )

    def forward(
        self,
        rgb_input: torch.Tensor,
        spectral_input: Optional[torch.Tensor] = None
    ) -> Union[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        """Forward pass.
        
        Args:
            rgb_input: RGB input tensor.
            spectral_input: Optional spectral input tensor.
        
        Returns:
            Classification logits (or tuple with aux logits during training).
        """
        if rgb_input.shape[2:] != self.img_input_size:
            logger.warning(
                f"InceptionV3 input RGB size {rgb_input.shape[2:]} != expected {self.img_input_size}. "
        ...
        if spectral_input is not None and self.spectral_conv1 is not None:
            if spectral_input.shape[2:] != self.img_input_size:
                logger.warning(
                    f"InceptionV3 input Spectral size {spectral_input.shape[2:]} != expected {self.img_input_size}."
                )
